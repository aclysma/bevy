use super::Schedule;
use crate::{resource::Resources, system::{ArchetypeAccess, System, ThreadLocalExecution, TypeAccess}, ParallelExecutorOptions};
...
pub struct MultitaskExecutor {
    ...
    clear_trackers: bool,
    mt_executor: Arc<multitask::Executor>,
}

impl Default for MultitaskExecutor {
    fn default() -> Self {
        Self::new("", true, &ParallelExecutorOptions::default())
    }
}

impl MultitaskExecutor {
    pub fn new(name: &str, clear_trackers: bool, executor_options: &ParallelExecutorOptions) -> Self {
        let mt_executor = multitask::Executor::default();

        let num_threads = if let Some(num_threads) = executor_options.num_threads {
            num_threads
        } else {
            8
        };
        ...
        for i in 0..num_threads {
            ...
            let ticker = mt_executor.ticker(move || u.unpark());

            let name = if name.is_empty() {
                format!("Bevy Executor {}", i)
            } else {
                format!("Bevy Executor ({}) {}", name, i)
            };

            let mut thread_builder = std::thread::Builder::new()
                .name(name);

            if let Some(stack_size) = executor_options.stack_size {
        ...
        Self {
            ...
            clear_trackers,//: true,
            mt_executor: Arc::new(multitask::Executor::default())
        }
    }
    ...
    pub fn run(&mut self, schedule: &mut Schedule, world: &mut World, resources: &mut Resources) {
        ...
        let executor = self.mt_executor.clone();
        for (stage_name, executor_stage) in schedule.stage_order.iter().zip(self.stages.iter_mut())
        {
            if let Some(stage_systems) = schedule.stages.get_mut(stage_name) {
                executor_stage.run(world, resources, stage_systems, schedule_changed, &*executor);
            }
        }
...
mod tests {
    ...
    fn cross_stage_archetype_change_prepare() {
        ...
        let mut executor = MultitaskExecutor::default();
    ...
    fn schedule() {
        ...
        fn run_executor_and_validate(
            executor: &mut MultitaskExecutor,
